...
lista_t *lista_insertar_en_posicion(lista_t *lista, void *elemento, size_t posicion) {
  ...
  if (posicion == 0) {
    ...
    nodo_t *nodo_anterior_a_modificar = nodo_en_posicion(lista, posicion - 1);
...
void *lista_quitar(lista_t *lista) {
  ...
  if (lista->cantidad == 1) {
    ...
    nodo_t *nuevo_fin = nodo_en_posicion(lista, lista->cantidad - 2);
    if (!nuevo_fin) {
      free(elemento);
      return NULL;
    }
...
void *lista_quitar_de_posicion(lista_t *lista, size_t posicion) {
  ...
  if (posicion == 0) {
    ...
    nodo_t *nodo_anterior_a_eliminar = nodo_en_posicion(lista, posicion - 1);
    if (!nodo_anterior_a_eliminar) return NULL;

    nodo_a_eliminar = nodo_anterior_a_eliminar->siguiente;
    nodo_anterior_a_eliminar->siguiente = nodo_a_eliminar->siguiente;
  }
